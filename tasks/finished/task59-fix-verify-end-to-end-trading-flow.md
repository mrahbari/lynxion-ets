When I run the system in **production mode**, a list of symbols is selected by the **watchers**, but unfortunately I can’t understand **why each signal is rejected or approved**.
For more clarification, explore the ./logs/* files.
Also, **after the initial evaluation**, under no circumstances is any symbol selected for **order placement**, which is very strange and most likely indicates that there is a problem.
Plus, review the following items and give me **tasks and acceptance criteria**.
...
* [ ] Better architecture: Each component now has a single responsibility. **SOLID principles** must be followed for coding.
...
* [ ] The system must be fully functional and able to place orders via the mentioned flow
