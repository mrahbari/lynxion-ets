"""E5.T5 (infra-only mechanical split): regime classifier + veto helper classes
extracted from ``regime_detector``.

Behavior-preserving — RegimeType (enum), ConfidenceBasedRegimeClassifier and
RegimeVetoMechanism moved verbatim (signatures, behavior, classifications UNCHANGED).
RegimeDetector (the public/singleton class) keeps composing these via re-export from
regime_detector, which preserves the public ``RegimeType`` import path. No layer move,
no trade surface, no logic change.
"""
import numpy as np
import pandas as pd
