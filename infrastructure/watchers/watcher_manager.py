...
from infrastructure.watchers.adapters.base_watcher import BaseWatcher
...
class WatcherManager:
    ...
    def register_watcher(self, name: str, watcher: BaseWatcher, 
    ...
    def _check_all_watchers_health(self):
        ...
        for name, watcher in self.watchers.items():
            try:
                ...
                if health_status.get('total_signals', 0) % 10 == 0:  # Every 10 signals
                    self.logger.info(f"Watcher {name} - Signals: {health_status.get('total_signals')}, "
    ...
    def _handle_watcher_error(self, name: str, watcher: BaseWatcher):
    ...
    def analyze_symbol(self, name: str, symbol: Symbol) -> Optional[Signal]:
        ...
        try:
            ...
            if signal:
                self.watcher_health[name].record_signal()
            return signal
        except Exception as e:
