...
class BaseWatcherAdapter(WatcherPort):
    ...
    def start(self):
        ...
        logger.info(f"Started watcher: {self.name} for symbol: {self.symbol.value}")
    
    def stop(self):
        """Stop the watcher"""
        self._is_running = False
        logger.info(f"Stopped watcher: {self.name} for symbol: {self.symbol.value}")
...
class MarketPulseWatcherAdapter(BaseWatcherAdapter):
    ...
    def analyze(self, symbol: Symbol) -> Optional[Signal]:
        ...
        logger.info(f"MarketPulse watcher analyzing {symbol.value}")
        ...
        if self.should_emit_signal(signal):
            ...
            logger.info(f"MarketPulse generated signal: {signal.signal_type.name}")
            return signal
        else:
            logger.info(f"MarketPulse signal filtered out due to similarity with last signal")
            return None
...
class VolatilityWatcherAdapter(BaseWatcherAdapter):
    ...
    def analyze(self, symbol: Symbol) -> Optional[Signal]:
        ...
        logger.info(f"Volatility watcher analyzing {symbol.value}")
        ...
        if self.should_emit_signal(signal):
            self.last_signal = signal
            logger.info(f"Volatility watcher generated signal: {signal.signal_type.name}")
            return signal
        else:
            return None
...
class TrendMTFWatcherAdapter(BaseWatcherAdapter):
    ...
    def __init__(self, symbol: Symbol):
        ...
        self.timeframes = ['1h', '4h', '1d']  # Multiple timeframes to analyze
    
    def analyze(self, symbol: Symbol) -> Optional[Signal]:
        """Analyze trends across multiple timeframes"""
        logger.info(f"TrendMTF watcher analyzing {symbol.value} across timeframes: {self.timeframes}")
        ...
        if self.should_emit_signal(signal):
            self.last_signal = signal
            logger.info(f"TrendMTF watcher generated signal: {signal.signal_type.name}")
            return signal
        else:
            return None
...
class AnomalyMLWatcherAdapter(BaseWatcherAdapter):
    ...
    def analyze(self, symbol: Symbol) -> Optional[Signal]:
        ...
        logger.info(f"AnomalyML watcher analyzing {symbol.value}")
        ...
        if self.should_emit_signal(signal):
            self.last_signal = signal
            logger.info(f"AnomalyML watcher generated signal: {signal.signal_type.name}")
            return signal
        else:
            return None
...
class OrderFlowWatcherAdapter(BaseWatcherAdapter):
    ...
    def analyze(self, symbol: Symbol) -> Optional[Signal]:
        ...
        logger.info(f"OrderFlow watcher analyzing {symbol.value}")
        ...
        if self.should_emit_signal(signal):
            self.last_signal = signal
            logger.info(f"OrderFlow watcher generated signal: {signal.signal_type.name}")
            return signal
        else:
            return None
