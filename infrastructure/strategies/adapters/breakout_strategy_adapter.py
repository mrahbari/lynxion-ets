...
class BreakoutStrategyAdapter(BaseStrategyAdapter):
    ...
    def generate_signal(self, symbol: Symbol) -> Optional[Signal]:
        """Generate signal using range breakout setups."""
        ...
        try:
            ...
            # Filter setups to only match NGBREAKOUT setups
            setup = next((s for s in setups if s.setup_type == "NGBREAKOUT"), None)
            if not setup:
                return None
    ...
    def evaluate_fused_signal(self, fused_signal: FusedSignal) -> Optional[ExecutionIntent]:
        """Evaluate fused signal using breakout confirmation and optimization."""
    ...
    def should_execute(self, fused_signal: FusedSignal) -> bool:
        """Verify the regime context is volatile/breakout."""
