...
class OIFootprintStrategyAdapter(BaseStrategyAdapter):
    ...
    def generate_signal(self, symbol: Symbol) -> Optional[Signal]:
        """Generate signal using Open Interest or volume footprint setups."""
        ...
        try:
            ...
            # Filter setups to only match NGOI_SETUP setups
            setup = next((s for s in setups if s.setup_type == "NGOI_SETUP"), None)
            if not setup:
                return None
    ...
    def evaluate_fused_signal(self, fused_signal: FusedSignal) -> Optional[ExecutionIntent]:
        """Evaluate fused signal using OI breakout confirmation and optimization."""
    ...
    def should_execute(self, fused_signal: FusedSignal) -> bool:
        """Verify the regime context matches breakout/volatile/trend."""
        from infrastructure.strategies.strategy_config import StrategyConfig
        if not StrategyConfig.get_strategy_enabled(self.name):
            return False

        # Support execution in volatile or breakout regimes
        is_suitable = any(term in fused_signal.regime_context.lower() for term in ['trend', 'volatile', 'breakout'])
        return is_suitable