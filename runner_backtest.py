...
def load_sample_strategy(strategy_name: str):
    ...
    if strategy_name in strategy_adapters:
        ...
        # Create a function that uses the strategy adapter's discipline rules
        def strategy_with_discipline(row, params):
            # Create a FusedSignal-like object from the row data
            from domain.entities.signal_entities import FusedSignal, SignalType
            from domain.value_objects import Symbol, Percentage
            from datetime import datetime
            from decimal import Decimal

            # Create a mock fused signal from the row data
            mock_symbol = Symbol(params.get('symbol', 'BTCUSDT'))
            mock_direction = 0  # Default direction
            mock_confidence = Percentage(Decimal('0.5'))  # Default confidence
            mock_regime = params.get('regime_context', 'stable')

            # Determine direction based on some indicators in the row if available
            if 'rsi' in row and not pd.isna(row['rsi']):
                rsi = row['rsi']
                if rsi < 30:
                    mock_direction = -0.5  # Oversold, potential buy
                elif rsi > 70:
                    mock_direction = 0.5   # Overbought, potential sell
                else:
                    mock_direction = 0.1 if rsi < 50 else -0.1  # Mild bias based on RSI

            # Create a mock fused signal
            from domain.entities.signal_entities import SignalType
            mock_dominant_bias = SignalType.NEUTRAL
            if mock_direction > 0.1:
                mock_dominant_bias = SignalType.BUY
            elif mock_direction < -0.1:
                mock_dominant_bias = SignalType.SELL
            ...
            # Use the strategy adapter's evaluate_fused_signal method which contains discipline rules
