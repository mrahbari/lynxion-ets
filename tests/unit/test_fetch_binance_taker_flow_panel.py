...
def test_build_accepts_custom_universe_and_task(tmp_path,monkeypatch):
    m=module(); monkeypatch.setattr(m,'list_archives',lambda *a:[]); monkeypatch.setattr(m,'normalize',lambda s,*a:{'unique_rows':120000,'missing_intervals':0,'conflicting_duplicates':0,'schema_violations':0,'numeric_violations':0,'timestamp_violations':0,'ohlc_violations':0,'flow_violations':0})
    report=m.build(tmp_path,1,('DOGEUSDT',),'TASK-X')
