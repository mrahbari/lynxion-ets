#!/usr/bin/env python3
"""Comprehensive portfolio backtest CLI shell (E2.T4).

Owns argument parsing and process I/O for multi-strategy, multi-symbol portfolio
backtesting, builds the composition root, resolves the portfolio backtester
factory port, and delegates orchestration to :class:`ValidatePortfolioUseCase`.
CLI arguments, console output, and exit codes are preserved verbatim from the
legacy ``runner_comprehensive_portfolio_backtest``.
"""
...
from application.use_cases.validate_portfolio import PortfolioBacktestRequest, ValidatePortfolioUseCase
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        with lifespan() as container:
            ...
            results = use_case.run_portfolio_backtest(request)
